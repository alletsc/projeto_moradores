...
def func_calc_percentual_valores_ausentes_linha(df):
    ...
    print(
        round(((missing_rows / total_rows) * 100), 2),
        "%",
...
def func_calc_percentual_valores_ausentes_coluna(df):
    ...
    mis_val_table_ren_columns = (
        ...
        .sort_values("% de Valores Ausentes", ascending=False)
        .round(2)
    )
    # Print
    print(
...
def fix_missing_ffill(df, col):
    ...
    print(
        f"{count} valores ausentes na coluna {col} foram substituídos usando o método de preenchimento progressivo."
    )
    return df[col]
...
def fix_missing_bfill(df, col):
    ...
    print(
        f"{count} valores ausentes na coluna {col} foram substituídos usando o método de preenchimento reverso."
    )
    return df[col]
...
def fix_missing_median(df, col):
    ...
    print(
        f"{count} valores ausentes na coluna {col} foram substituídos por seu valor de mediana {median}."
    )
    return df[col]
